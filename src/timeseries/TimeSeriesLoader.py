...
def uv_load(base_path, dataset_name):
    try:
        ...
        print("Done reading " + dataset_name + " Training Data...  Samples: " +
              str(train["Samples"]) + "   Length: " + str(train["Size"]))
        print("Done reading " + dataset_name + " Testing Data...  Samples: " +
              str(test["Samples"]) + "   Length: " + str(test["Size"]))
        print()
...
def mv_load(dataset_name, useDerivatives):
    ...
    try:
        ...
        train_raw = pd.read_csv(
            (mv_dir + dataset_name + "\\" + dataset_name + "_TRAIN3"), sep=" ", header=None)
        test_raw = pd.read_csv(
            (mv_dir + dataset_name + "\\" + dataset_name + "_TEST3"), sep=" ", header=None)

        train["Type"] = "MV"
        train["Samples"] = int(train_raw.iloc[-1, 0])
        train["Dimensions"] = 2 * (train_raw.shape[1] -
                                   3) if useDerivatives else train_raw.shape[1] - 3
        train["Labels"] = []

        test["Type"] = "MV"
        test["Samples"] = int(test_raw.iloc[-1, 0])
        test["Dimensions"] = 2 * (test_raw.shape[1] -
                                  3) if useDerivatives else test_raw.shape[1] - 3
        test["Labels"] = []
        ...
        print("Done reading " + dataset_name + " Training Data...  Samples: " +
              str(train["Samples"]) + "   Dimensions: " + str(train["Dimensions"]))
        print("Done reading " + dataset_name + " Testing Data...  Samples: " +
              str(test["Samples"]) + "   Dimensions: " + str(test["Dimensions"]))
        print()
